...
Scan Node.js projects for known vulnerabilities using the OSV.dev database.

### Usage

```bash
npx tsx src/cli/index.ts scan <path> [--all]
```
...
| Command | Description |
|---------|-------------|
| `scan <path>` | Scan for vulnerable dependencies with optional reachability filtering |
| `fix <path>` | Scan and create a GitHub PR with安全的版本更新 |

### Options

- `--all` — Show all vulnerabilities, including those not reachable from application code
...
| Severity | Vulnerability severity (CVSS/GHSA) |
| Safe Version | Minimum patched version |
...
npx tsx src/cli/index.ts scan ./path/to/project --all
```
...
Two runnable demo fixtures ship in [`examples/`](./examples/):

- **`vigilops-demo`** — pins `lodash@4.17.15`, imports it from `index.js`. Exercises the lodash prototype-pollution CVE class.
- **`vigilops-demo-express`** — pins `express@4.18.2` (qs prototype-pollution class), includes an unreached `minimist@0.0.8` dep to demonstrate suppression. Express is reachable; minimist is not.
...
# show all (including not-reachable) rows
npx tsx ../../src/cli/index.ts scan . --all
```

Requires outbound HTTPS to `api.osv.dev`. See each fixture's README for details.
